                        ...
                        <!-- ROW-1 OPEN -->
                         <form method="post" enctype="multipart/form-data">
                           ...
                           if(isset($_REQUEST['submit'])){
                            ...
                            $targetphoto="../photo/";
                            $targetphoto=$targetphoto.$_FILES["photo"]['name'];

                            $coverphoto = $_FILES["coverphoto"]['name'];
                            $target="coverphoto/";
                            $target=$target.$_FILES["coverphoto"]['name'];

                            $check=mysqli_query($conn, "SELECT * FROM shop WHERE uin='$uin'");
                            $checkrows=mysqli_num_rows($check);
                            if($checkrows>0){
                              echo"<script>alert('This product already exists in Database')</script>";}
                              else{
                                if(move_uploaded_file($_FILES["photo"]['tmp_name'], $targetphoto)>0){
                                if(move_uploaded_file($_FILES["coverphoto"]['tmp_name'], $target)>0){
                                    
                                $sql = "INSERT INTO shop (uin, productname, quantity, `description`, category, price, discount, `date`, `status`, `photo`, coverphoto) VALUES ('$uin','$productname','$quantity','$description','$category','$price','$discount','$date','$status','$photo','$coverphoto')";
        ...
        <select type="text" class="form-control" name="category">
            ...
            ?>
                                        ...
                                        <div class="row mb-4">
                                            ...
                                            <div class="col-md-9">
                                                <input type="file" class="form-control" name="photo" accept=".jpg,.jpeg,.png,.PNG,.JPG,.JPEG" >
                                            </div>
                                        </div>
                                        <div class="row mb-4">
                                            <label class="col-md-3 form-label">Cover Photo</label>
                                            <div class="col-md-9">
                                                <input type="file" class="form-control"  name="coverphoto" accept=".jpg,.jpeg,.png,.PNG,.JPG,.JPEG">
                                            </div>
                                        </div>
                                        <!--Row-->
                                        <!--End Row-->
                                    </div>
                                    <input type="submit" class="card-footer btn-primary btn" name="submit" value="Upload">
                                </div>
                            </div>
                        </div></form>
                        <!-- /ROW-1 CLOSED -->
